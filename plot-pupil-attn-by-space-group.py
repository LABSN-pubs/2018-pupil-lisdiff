...
# plot style. cannot change cycler after axes created via plt.style.use(...),
# so we define colors for second row of subplots as a cycler object
style_files = ('font-libertine.yaml', 'garnish.yaml')
plt.style.use([op.join(styledir, sf) for sf in style_files])
reds = cycler(color=[prp, pch])
blues = cycler(color=[blu, cyn])
...
renamer = dict(ldiff='listening\ndifficulty', control='age-matched\ncontrol')
...
fig.subplots_adjust(left=0.08, right=0.92, bottom=0.15, top=0.88,
...
for group_ix, (group_name, group) in enumerate(groups.items()):
    ...
    for space_ix, (s_cond_name, s_cond) in enumerate(spatial_conds.items()):
        ax = axs[group_ix, space_ix]
        these_colors = []
        ax.set_prop_cycle((blues, reds)[group_ix])
        ...
        for attn_ix, (a_cond_name, a_cond) in enumerate(attn_conds.items()):
            ...
            if not space_ix:
                ypos = [0.95, 0.75][attn_ix] * ymax
                ax.annotate(a_cond_name, (-0.2, ypos), xytext=(0, 0),
                            color=this_color, textcoords='offset points',
        ...
        plot_trial_timecourse(ax, x, y, w, h, boxcolors, these_colors[::-1])

        # stats
        if plot_signif:
            # load stats
            fname = (f'capd_stats_{deconv}_maintain-vs-switch_{s_cond_name}_'
                     f'{group_name}.yaml')
        ...
        if not space_ix:
            ...
            ax.set_ylabel(renamer.get(group_name).upper(),
                          rotation=-90, labelpad=36)
...
if savefig:
    fname = f'capd-pupil-{deconv}-attn-by-space-group.pdf'
